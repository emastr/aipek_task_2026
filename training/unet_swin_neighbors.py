if __name__ == "__main__":
    ...
    neighbor_loader = NeighborLoader(
        ...
        label_dir = f"{CONSTANTS.NORM_DATA_PATH_TRAIN_OUT}",
        channels = channels, # Number of slices per chunk
        neighbors = 2, # Number of neighboring slices to include on each side
        num_slices = num_slices, # Number of slices per chunk
        device = "cuda",
        size = size
    )
    ...
    def validation_plots(net, loader, path, epoch):
        ...
        for i in range(3):
            ...
            x_nei = neigbor_loader.get_neighbors(x_val, ignore_idx = idx_batch) # make sure to ignore the current slice when getting neighbors
            ynet_val = net(x_val)
