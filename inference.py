...
class CoWSegBaseline():
    ...
    def create_final_det_box(self, input_img_path: Path) -> dict:
        ...
        # For low-confidence domains (e.g. NCCT), thresholding may remove all
        # boxes. Fall back to top-scoring boxes so ROI extraction can proceed.
        if len(X_min) == 0:
            if num_predictions > 0:
                print(
                    f"No boxes passed threshold {self.threshold}. "
                    f"Falling back to top {top_preds} boxes regardless of threshold."
                )
                for k in range(1, top_preds + 1):
                    box = pred_boxes[idx[-k]]
                    X_min.append(int(box[4]))
                ...
                print("No detection boxes found. Falling back to full-image ROI.")
                img = sitk.ReadImage(input_img_path)
                sx, sy, sz = img.GetSize()
                location = (0, 0, 0)
                endpt = (sx - 1, sy - 1, sz - 1)
                size = (sx, sy, sz)
                prediction_meta = {
                    "top_num_preds": 0,
                    "location": location,
    ...
    def crop_image_for_segmentation(self, input_img_path: Path, remove_old_rois: bool=False):
        ...
        # Store clamped ROI metadata so padding uses exactly the same region.
        prediction_meta["location"] = (int(start_x), int(start_y), int(start_z))
        prediction_meta["endpoint"] = (int(end_x), int(end_y), int(end_z))
        prediction_meta["size"] = (
            int(end_x - start_x + 1),
            int(end_y - start_y + 1),
            int(end_z - start_z + 1),
        )
    ...
    def pad_prediction(self, input_img_path: Path):
        ...
        # Clamp and match insertion extents to both mask and prediction shape.
        x_min = max(0, x_min)
        y_min = max(0, y_min)
        z_min = max(0, z_min)
        x_max = min(shape[0] - 1, x_max)
        y_max = min(shape[1] - 1, y_max)
        z_max = min(shape[2] - 1, z_max)

        tx = min(max(0, x_max - x_min + 1), pred_arr.shape[0])
        ty = min(max(0, y_max - y_min + 1), pred_arr.shape[1])
        tz = min(max(0, z_max - z_min + 1), pred_arr.shape[2])

        if tx > 0 and ty > 0 and tz > 0:
            mask_array[x_min:x_min+tx, y_min:y_min+ty, z_min:z_min+tz] = pred_arr[:tx, :ty, :tz]
        else:
            print(f"Warning: empty ROI insertion for {input_img_path.name}; skipping pad insertion.")
